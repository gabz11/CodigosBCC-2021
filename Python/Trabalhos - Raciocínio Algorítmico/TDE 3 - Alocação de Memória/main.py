...
def conversao(matriz_linha):
    posicao = 0
    nova_linha = []
    nova_matriz = []

    for i in range(0, len(matriz_linha)):
        nova_linha.append(matriz_linha[i])
        posicao += 1

        if posicao == n_colunas:
...
def first_fit():
    ...
    for i in range(0, len(matriz)):
        for j in range(0, len(matriz[i])):
            matriz_linha.append(matriz[i][j])

    posicao_considerada = -1
    qtd_vazios = 0

    for i in range(0, len(matriz_linha)):
        termo = matriz_linha[i]
        if termo == ' ':
            qtd_vazios += 1
            if posicao_considerada == -1:
                posicao_considerada = i
            if qtd_vazios == tamanho:
                while posicao_considerada < i + tamanho:
                    matriz_linha[posicao_considerada] = 'X'
                    posicao_considerada += 1
                    tamanho -= 1
                    if tamanho == 0:
                        break
    ...
    nova_matriz = conversao(matriz_linha)
